...
export async function POST(req: NextRequest) {
  try {
    ...
    const subscriptionNames = ['Kansas', 'Panda', 'Kung', 'Braums', 'Netflix', 'Prime', 'HBO']; // Add more subscription names as needed
    let index = 0;

    while (true) {
      const keyword = formData.get(`keywords[${index}]`);
      if (keyword === null) {
        // If no more keywords with the current index, break the loop
        break;
      }
      subscriptionNames.push(keyword as string);
      index++;
    }
